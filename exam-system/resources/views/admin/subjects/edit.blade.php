    ...
    <form method="POST" action="{{ route('admin.subjects.update', $subject) }}"
        ...
        <div class="mb-4">
            ...
            <select name="exam_category_id" required
                ...
                @foreach($categories as $cat)
                    <option value="{{ $cat->id }}"
                        {{ old('exam_category_id', $subject->exam_category_id) == $cat->id ? 'selected' : '' }}>
                        {{ $cat->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <textarea name="description" rows="3"
                      class="w-full px-3 py-2 border border-gray-300 rounded-lg">{{ old('description', $subject->description) }}</textarea>
        </div>
