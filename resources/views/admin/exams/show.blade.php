        ...
        <div class="bg-indigo-700 px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                ...
                <p class="text-indigo-200 text-xs mt-0.5">
                    {{ $stats['completed_attempts'] }} submitted &nbsp;·&nbsp;
                    {{ $stats['published_results'] }} published &nbsp;·&nbsp;
                    {{ $stats['unpublished_results'] }} under review
                    @if($stats['no_result_count'] > 0)
                        &nbsp;·&nbsp; <span class="text-red-300 font-semibold">{{ $stats['no_result_count'] }} no result</span>
                    @endif
                </p>
            </div>
                            ...
                            <tr class="hover:bg-gray-50 transition" id="attempt-row-{{ $attempt->id }}">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        {{-- Preview attempt button — always available --}}
                                        ...
                                        {{-- Publish button — only for unpublished results --}}
                    ...
                    <div id="enrolled-students-list" class="divide-y divide-gray-100 max-h-[400px] overflow-y-auto">
                        @forelse($exam->enrolledStudents as $student)
                            ...
                            <div id="enrolled-row-{{ $student->id }}" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50">
                                ...
                                <button onclick="unenrollStudent({{ $student->id }})" class="shrink-0 text-red-400 hover:text-red-600 text-xs px-2 py-1 rounded transition">✕</button>
                            </div>
                            @endif
                        @empty
...
<script>
    ...
    function searchQuestions(page=1){
        ...
        .catch(err=>{document.getElementById('question-bank-list').innerHTML=`<div class="px-6 py-8 text-center text-red-500 text-sm"><p class="text-2xl mb-2">⚠️</p><p>${escapeHtml(err.message)}</p><button onclick="searchQuestions(1)" class="mt-3 text-blue-600 hover:underline text-xs">Retry</button></div>`;});
    }

    function renderBankResults(questions){
        const list=document.getElementById('question-bank-list');
        if(!questions?.length){list.innerHTML='<div class="px-6 py-10 text-center text-gray-400 text-sm">No questions found.</div>';return;}
        ...
        list.innerHTML=questions.map(q=>`
            <div id="bank-row-${q.id}" class="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition">
                ...
                <button onclick="addQuestion(${q.id})" id="add-btn-${q.id}" class="shrink-0 bg-green-500 hover:bg-green-600 text-white text-xs px-3 py-1.5 rounded-lg transition font-medium">➕ Add</button>
            </div>`).join('');
    }
    ...
    function checkEmptyAssigned(){const l=document.getElementById('assigned-questions-list');if(!l.querySelector('[id^="assigned-row-"]'))l.innerHTML=`<div id="no-questions-msg" class="px-6 py-12 text-center text-gray-400"><p class="text-4xl mb-2">📭</p><p>No questions assigned.</p></div>`;}
